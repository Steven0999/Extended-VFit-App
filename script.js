...
let state = {
    ...
    mission: "",
    wins: [],
    addons: { steps: false, habits: false },
    stepGoal: 10000,
    dailySteps: 0,
    habitList: [],
    habitsCompleted: {}
...
window.onload = () => {
    ...
    document.getElementById('config-cals-goal').value = state.goals.calories;
    ...
    // Added from first code: Sync UI with stored settings
    document.getElementById('toggle-steps').checked = state.addons.steps;
    document.getElementById('toggle-habits').checked = state.addons.habits;
    document.getElementById('config-step-goal').value = state.stepGoal;
    document.getElementById('config-habits-list').value = state.habitList.join(', ');
    document.getElementById('settings-mission').value = state.mission;
    document.getElementById('settings-wins').value = state.wins.join('\n');
...
function archiveDailyNutrition() {
    ...
    // Storing the full day intake summary
    const archiveEntry = {
        id: Date.now(),
        date: state.viewDate,
...
function renderDashboard() {
    ...
    // Added from first code: Mission & Wins
    document.getElementById('mission-display').innerText = state.mission || "Define your mission in settings...";
    const winsList = document.getElementById('wins-list');
    winsList.innerHTML = '';
    if (state.wins.length === 0) {
        winsList.innerHTML = '<li class="text-sm">• No wins recorded yet</li>';
    } else {
        state.wins.forEach(win => {
            const li = document.createElement('li');
            li.className = "text-sm flex items-center";
            li.innerHTML = `<span class="text-indigo-500 mr-2">✔</span> ${win}`;
            winsList.appendChild(li);
        });
    }

    // Added from first code: Add-on Visibility
    document.getElementById('card-steps').classList.toggle('hidden', !state.addons.steps);
    document.getElementById('card-habits').classList.toggle('hidden', !state.addons.habits);

    if (state.addons.steps) {
        document.getElementById('current-steps').innerText = state.dailySteps;
        document.getElementById('goal-steps').innerText = state.stepGoal;
        const pct = Math.min(100, Math.round((state.dailySteps / state.stepGoal) * 100));
        document.getElementById('step-progress').innerText = pct + '%';
    }

    if (state.addons.habits) {
        const habitDiv = document.getElementById('habits-display');
        habitDiv.innerHTML = '';
        state.habitList.forEach(habit => {
            const isDone = state.habitsCompleted[habit];
            const btn = document.createElement('button');
            btn.className = `px-3 py-2 rounded-lg text-xs font-bold transition-all ${isDone ? 'bg-indigo-600 text-white' : 'bg-slate-200 text-slate-500'}`;
            btn.innerText = habit;
            btn.onclick = () => toggleHabit(habit);
            habitDiv.appendChild(btn);
        });
        if (state.habitList.length === 0) {
            habitDiv.innerHTML = '<p class="text-xs text-slate-500">No habits set in Settings</p>';
        }
    }
}
...
// --- Added from first code: SETTINGS & CONFIG ---
function updateAddonUI() {
    state.addons.steps = document.getElementById('toggle-steps').checked;
    state.addons.habits = document.getElementById('toggle-habits').checked;
    
    document.getElementById('goal-steps-config').classList.toggle('hidden', !state.addons.steps);
    document.getElementById('goal-habits-config').classList.toggle('hidden', !state.addons.habits);
...
function saveConfig() {
    state.stepGoal = parseInt(document.getElementById('config-step-goal').value) || 10000;
    state.goals.calories = parseInt(document.getElementById('config-cals-goal').value) || 2000;
    
    const habitsInput = document.getElementById('config-habits-list').value;
    state.habitList = habitsInput.split(',').map(h => h.trim()).filter(h => h !== "");
    
    saveState();
}

function saveReflection() {
    state.mission = document.getElementById('settings-mission').value;
    const winsText = document.getElementById('settings-wins').value;
    state.wins = winsText.split('\n').filter(w => w.trim() !== "");
    
    saveState();
    alert("Daily Reflection Updated!");
    renderDashboard();
}

// --- Added from first code: FEATURE LOGIC ---
function updateSteps() {
    state.dailySteps = parseInt(document.getElementById('step-input').value) || 0;
    saveState();
    renderDashboard();
}

function toggleHabit(habit) {
    state.habitsCompleted[habit] = !state.habitsCompleted[habit];
    saveState();
    renderDashboard();
}

// --- Added from first code: UTILS ---
function clearData() {
    if (confirm("Delete all your progress? This cannot be undone.")) {
        localStorage.removeItem('fittrack_combined_v8');
        location.reload();
    }
            }
